...
/**
 * Core WhatsApp sender via Twilio WhatsApp Business API
 * Requires a Twilio WhatsApp-enabled number (sandbox or approved business number).
 * `to` must be in E.164 format (e.g. +15041234567) — the whatsapp: prefix is added automatically.
 */
export async function sendWhatsApp(to: string, body: string): Promise<SMSResult> {
  if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN) {
    console.error('[Twilio WhatsApp] Missing credentials. Set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN.');
    return { success: false, error: 'Twilio credentials not configured' };
  }

  const fromNumber = TWILIO_WHATSAPP_NUMBER || TWILIO_PHONE_NUMBER;
  if (!fromNumber) {
    console.error('[Twilio WhatsApp] No WhatsApp number configured. Set TWILIO_WHATSAPP_NUMBER.');
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : 'Unknown error';
    console.error('[Twilio WhatsApp] sendWhatsApp error:', message);
    return { success: false, error: message };
  }
}
...
export async function sendConsultationReminderWhatsApp(opts: {
  ...
  const consultation = opts.appointmentType ?? 'Paralegal Consultation';
  const meetUrl = opts.meetingLink ?? 'https://broussardlegalservices.com/availability';

  const body = opts.reminderType === '24hr'
    ? `*Maggi May Broussard Legal Services*\n\nHi ${firstName} 👋 Your *${consultation}* is *TOMORROW*!\n\n📅 ${opts.appointmentDate}\n⏰ ${opts.appointmentTime}\n🔗 ${meetUrl}\n\nNeed to reschedule? Visit broussardlegalservices.com/availability\n\n_Reply STOP to opt out_`
    : `*Maggi May Broussard Legal Services*\n\nHi ${firstName} ⏰ Your *${consultation}* starts in *1 HOUR* at ${opts.appointmentTime}.\n\n🔗 ${meetUrl}\n\nPlease make sure you're ready to connect.\n\n_Reply STOP to opt out_`;

  return sendWhatsApp(opts.to, body);
}

/**
 * Send case update notification via WhatsApp
 */
export async function sendCaseUpdateWhatsApp(opts: {
  ...
  const firstName = opts.clientName.split(' ')[0];
  const portalUrl = opts.portalLink ?? 'https://broussardlegalservices.com/portal/cases';

  const body =
    `*Maggi May Broussard Legal Services*\n\nHi ${firstName}, there's an update on your case *${opts.caseName}*:\n\n${opts.updateMessage}\n\n🔗 View details: ${portalUrl}\n\n_Reply STOP to opt out_`;
...
export async function sendPaymentReminderWhatsApp(opts: {
  ...
  const body = opts.isOverdue
    ? `*Maggi May Broussard Legal Services*\n\n⚠️ Hi ${firstName}, invoice *#${opts.invoiceNumber}* for *${opts.amount}* is *OVERDUE* (was due ${opts.dueDate}).\n\n💳 Pay now: ${portalUrl}\n\n_Reply STOP to opt out_`
    : `*Maggi May Broussard Legal Services*\n\nHi ${firstName} 📋 Friendly reminder: invoice *#${opts.invoiceNumber}* for *${opts.amount}* is due on *${opts.dueDate}*.\n\n💳 Pay securely: ${portalUrl}\n\n_Reply STOP to opt out_`;

  return sendWhatsApp(opts.to, body);
}

/**
 * Send client notification via WhatsApp (general purpose)
 */
export async function sendClientNotificationWhatsApp(opts: {
  ...
  const firstName = opts.clientName.split(' ')[0];
  const linkLine = opts.actionLink ? `\n\n🔗 ${opts.actionLabel ?? 'View details'}: ${opts.actionLink}` : '';

  const body =
    `*Maggi May Broussard Legal Services*\n\n*${opts.subject}*\n\nHi ${firstName},\n\n${opts.message}${linkLine}\n\n_Reply STOP to opt out_`;
